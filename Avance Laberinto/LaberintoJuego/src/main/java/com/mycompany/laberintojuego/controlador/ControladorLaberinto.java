...
public class ControladorLaberinto implements KeyListener {
    ...
    public void ganar() {
    if (areaJuego.meta()) {   // si el capibara llega a las coordenas indicadas
        guiLaberinto.showMessage(areaJuego.ganar()); // mensaje que dice que gano
    }
}
    
    public boolean isGanar(){
        return areaJuego.meta();  //Metodo que retorna si el personaje llego a la meta
    }
